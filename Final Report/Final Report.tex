...
To address this challenge, this project evaluates the predictive models chosen using a custom
FN \& FP Score that combines the false negative rate and false positive
rate with adjustable weights. This scoring system allows model rankings to
reflect the manufacturing tradeoff between missed defects and false alarms.
Multiple imbalance-handling strategies are compared, including random
oversampling, random undersampling, Synthetic Minority Oversampling
Technique (SMOTE), and class-weighted learning. Several machine learning
models are evaluated, including Logistic Regression, Decision Trees,
Random Forests, and boosting models. The goal is to develop a reliable
defect detection framework that improves minority-class prediction and
...
\subsection*{Preferred Metrics}
The dataset comprises 229,999 test records with only 458 defective samples (0.2\%). In manufacturing, missing a defect (false negative) is significantly more costly than incorrectly flagging a healthy part. Consequently, accuracy and ROC AUC can be misleading metrics for severe class imbalance \cite{imani2026roc}. Instead, we prioritize recall, precision, Area Under the Precision-Recall Curve (PR-AUC), and the F1-minority score.
 
Using the regular F1 score masks minority class performance because the majority class dominates the aggregation:
$$F1_{regular} = 2 \cdot \frac{P_{overall} \cdot R_{overall}}{P_{overall} + R_{overall}}$$
 
Computing precision and recall exclusively for the defective class avoids this masking effect \cite{sciencedirect_minority}:
$$F1_{minority} = 2 \cdot \frac{P_{minority} \cdot R_{minority}}{P_{minority} + R_{minority}}$$
 
\subsection*{Class Imbalance Strategies}
To address the imbalance, we evaluate four dataset manipulation techniques: Random Oversampling (duplicating minority samples), Random Undersampling (removing majority samples), SMOTE (synthetic sample generation), and Class-Weighted training (adjusting learning objectives to penalize minority-class errors).
 
\subsection*{Model Selection}
We evaluate simple models alongside complex ensembles to measure the trade-off between interpretability and predictive performance.
...
Random Forest aggregates multiple trees trained on bootstrap samples and random feature subsets, yielding a majority vote prediction:
$$\hat{y} = mode\{\hat{y}^{(1)}, \hat{y}^{(2)}, \dots, \hat{y}^{(N)}\}$$
We evaluate both Balanced Random Forest (internal undersampling per tree) and Cost-Sensitive Random Forest (class-weighted impurity).
...
Our experimental phase evaluates 120 unique scenarios across three components:
\begin{itemize}
    \item \textbf{Model Benchmark:} Comparing algorithms across predictive power and transparency.
    \item \textbf{Class Imbalance Strategy Ablation:} Quantifying the impact of the four sampling and weighting strategies on the decision boundary.
    \item \textbf{Data Split Comparison:} Testing model generalization across 90/10, 80/20, and 70/30 train/test split ratios.
\end{itemize}

\subsection{"FN \& FP Score" System for Model Evaluation}
...
The initial benchmark utilized default parameters. A secondary run utilized hyperparameters tuned via 5-Fold Cross Validation on the baseline 80/20 split:
...
Data split ratios had minimal effect on aggregate performance. The class imbalance treatment was the overwhelming driver of the precision-recall tradeoff.
 
\paragraph{Random Undersampling:} This strategy caused severe distribution mismatch. All models hit a training precision of 1.0 but dropped below 10\% test precision. Memorizing an artificially shrunk training distribution caused predictions to flood toward the minority class upon encountering natural test imbalance.
 
\paragraph{Baseline (Unweighted):} XGBoost and CatBoost performed strongly without manipulation, yielding high train-test precision alignment ($\sim$96–100\% test) and 70–80\% recall. LightGBM, however, demonstrated severe instability, degrading dramatically on smaller (70/30) training sets.
 
\paragraph{Class-Weighted:} For XGBoost, CatBoost, and LightGBM, class-weighting provided a clean, low-complexity lift in recall ($\sim$76–80\%) while maintaining $>92\%$ test precision. AdaBoost failed to respond to class weighting.
 
\paragraph{SMOTE \& Random Oversampling:} XGBoost, LightGBM, and CatBoost achieved their best overall F1 scores under Random Oversampling, with 93–100\% test precision and 73–84\% recall. Under SMOTE, models showed a moderate train-test precision gap (signaling slight overfit to synthetic data) but remained competitive.
 
\paragraph{AdaBoost Limitations:} Across all resampling techniques that inflated training size (SMOTE, Oversampling), AdaBoost's test precision collapsed (9–37\%) while runtimes ballooned to over 100$\times$ slower than CatBoost or XGBoost.
 
\subsection*{Conclusion}
Sampler choice, rather than data split, dictates performance. XGBoost and CatBoost proved to be the most robust algorithms across all strategies. LightGBM requires an active balancing strategy to function reliably, and Random Undersampling broadly leads to overfitting. Ultimately, class-weighted or oversampled gradient boosting models provide the optimal balance for minimizing false negatives in highly imbalanced manufacturing data.
