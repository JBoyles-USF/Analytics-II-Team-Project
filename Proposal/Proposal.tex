...
While Balanced RF handles imbalance by rsampling the data for each tree, cost-sensitive RF address the same issue by applying class weighted impurity so that errors on defective samples receive higher penalties (One application of the Class weighted strategy). Evaluating both RF variants allows us to compare the best model for detection performance.
...
\begin{enumerate}
    \item \textbf{Data Inspection:} Load the dataset and analyze the distribution.
    \item \textbf{Exploratory Data Analysis:} Identify sensor ranges, correlations, and outliers across the 30 numerical predictors to understand baseline behavior.
    \item \textbf{Partitioning:} Split data into training (80\%), validation (10\%), and test sets (10\%).
    \item \textbf{Imbalance Handling:} Apply strategies including Random Oversampling, Undersampling, SMOTE, and Class-Weighted training within cross-validation folds to prevent data leakage.
    \item \textbf{Simple Modeling:} Train Logistic Regression and Decision Trees to establish interpretable reference performance and identify primary predictors.
    \item \textbf{Ensemble Modeling:} Train Random Forest (using the Balanced variant) and Boosting methods (AdaBoost/XGBoost) to capture complex non-linear interactions.
    \item \textbf{Performance Evaluation:} Assess all models using metrics that reflect the high cost of false negatives: Precision, Recall, F1-minority, and PR-AUC.
    \item \textbf{Ablation Study:} Conduct a systematic comparison by "swapping out" imbalance strategies to identify which method provides the most stable defect detection.
    \item \textbf{Interpretability Analysis:} Research and utilize SHAP values and permutation importance to rank the most influential sensors and explain model decisions to maintenance teams.
  \end{enumerate}
\begin{table}[h]
    \centering
...
\begin{itemize}
    \item \textbf{Minority Class Sensitivity:} Prioritizing Recall and F1-minority to ensure rare defects are not overlooked.
   
    \item \textbf{Metric Integrity:} Using PR-AUC to provide a truthful assessment, as ROC-AUC tends to be overly optimistic when the minority class is <3\% \cite{imani2026roc}.
   
    \item \textbf{Architectural Complexity:} Quantifying the cost of interpretability using specific indicators:
    \begin{itemize}
        ...
        \item \textbf{Ensembles:} Number of estimators, feature importance stability, and process times.
    \end{itemize}
   
    \item \textbf{Economic Impact:} Tracking False Negatives (FN) as a  proxy for the cost of manufacturing failure, while process times for complex models will reflect process resource costs.
\end{itemize}
